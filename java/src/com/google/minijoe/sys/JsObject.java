  ...
  public void evalNative(int index, JsArray stack, int sp, int parCount) {
    ...
    switch(index) {
      ...
      case ID_INIT_DATE:
        // reset to defaults
        if (isConstruction(stack, sp)){
          JsDate d3 = (JsDate) this;
          if (parCount == 1) {
            d3.time.setTime(new Date((long) stack.getNumber(sp + 2)));
          } else if (parCount > 1){
            d3.time.setTime(new Date(0));
            int year = stack.getInt(sp + 2);
            if (year >= 0 && year <= 99) { 
              year += 1900;
            }
            d3.setDate(false, year, 
                           stack.getNumber(sp + 3), 
                           stack.getNumber(sp + 4));
            
            d3.setTime(false, stack.getNumber(sp + 5), 
      ...
      case ID_SPLIT:
        ...
        if (sep.length() == 0) {
          ...
          for (int i9 = 0; i9 < limit; i9++) {
            a.setObject(i9, s.substring(i9, i9+1));
          }
        }
        else {
          int cut0 = 0;
