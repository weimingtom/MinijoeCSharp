...
public class CodeGenerationVisitor implements Visitor {
    ...
    try {
      if (variables != null) {
        ...
        for (int i = 0; i < variables.length; i++) {
          dos.writeShort((short) ((Integer) globalStringMap.get(variables[i].str)).intValue());
        }
      }
    } catch (IOException e) {
      throw new CompilerException(e);
  ...
  void writeJump(int op, Object baseObj, String type) {
    int pos = codeStream.size() + 1;

    if (baseObj instanceof String) {
      type = type + "-" + baseObj;
    } else if (baseObj instanceof Node) {
      type = type + "=" + baseObj.hashCode();
  ...
  private int getStringLiteralIndex(String str) {
    int i = stringLiterals.indexOf(str);
    if (i == -1) {
      i = stringLiterals.size();
      addToGlobalStringTable(str);
      stringLiterals.addElement(str);
    }
    return i;
  }
  ...
  public ProgramNode visit(ProgramNode program) throws CompilerException {
  ...
  public Statement visit(LabelledStatement statement) throws CompilerException {
    labelSet.addElement(statement.identifier.str);
    statement.statement.visitStatement(this);
    return statement;
  }
}
